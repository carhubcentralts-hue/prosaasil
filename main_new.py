...
# -*- coding: utf-8 -*-
"""
HEBREW AI CALL CENTER - FINAL WORKING VERSION
Fixed all Twilio Hebrew issues with Play verb
"""
...
# Configuration
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'hebrew-ai-fixed-2025')
...
def hebrew_incoming_call():
    ...
    logger.info(f"🎯 BREAKTHROUGH CALL: {call_sid} from {from_number}")
...
@app.route("/webhook/incoming_call_hebrew_fixed", methods=['POST'])
def hebrew_incoming_call_new():
    """Alternative route - same fix"""
    return hebrew_incoming_call()

@app.route("/webhook/handle_recording", methods=['POST'])
def hebrew_handle_recording():
    """Handle Hebrew recording"""
    call_sid = request.form.get('CallSid', 'unknown')
    recording_url = request.form.get('RecordingUrl')
    
    logger.info(f"🎙️ HEBREW RECORDING: {call_sid}")
...
# ============================================
# CREATE HEBREW GREETING FILE IF MISSING
# ============================================

def ensure_hebrew_files():
    """Create Hebrew MP3 files if missing"""
    static_dir = os.path.join(os.path.dirname(__file__), 'server', 'static')
    os.makedirs(static_dir, exist_ok=True)
    
    greeting_file = os.path.join(static_dir, 'greeting.mp3')
    
    if not os.path.exists(greeting_file) or os.path.getsize(greeting_file) < 1000:
        logger.info("Creating Hebrew greeting file...")
        try:
            from gtts import gTTS
            text = "שלום וברכה! הגעתם לשי דירות ומשרדים בעמ. אני כאן לעזור לכם למצוא את הנכס המושלם. במה אוכל לעזור לכם?"
            tts = gTTS(text=text, lang='iw', slow=False)
            tts.save(greeting_file)
            logger.info(f"✅ Hebrew greeting created: {os.path.getsize(greeting_file)} bytes")
        except Exception as e:
            logger.error(f"❌ TTS failed: {e}")
            # Create placeholder
            with open(greeting_file, 'wb') as f:
                f.write(b'Hebrew MP3 placeholder')
