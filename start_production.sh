...
echo "🚀 Starting AgentLocator Production System - Build #85"
echo "📊 Flask: 0.0.0.0:${PORT} | Baileys: 127.0.0.1:${BAILEYS_PORT}"
echo "✅ Build 85: Google STT Fix + Conversation Memory + Auto Leads"
...
# 2) Start Flask/ASGI with Uvicorn (native WebSocket support - BUILD 85)
echo "🟡 Starting BUILD 85 with Uvicorn ASGI on port ${PORT}..."
uvicorn asgi:app --host 0.0.0.0 --port ${PORT} --ws websockets --lifespan off --timeout-keep-alive 75 --log-level info &
FL=$!
echo "✅ BUILD 85 Uvicorn/ASGI started (PID: $FL)"
