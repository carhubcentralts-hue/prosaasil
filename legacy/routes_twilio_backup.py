...
def incoming_call():
    """Handle incoming Twilio call - Hebrew greeting works now!"""
    try:
        ...
        # Always use simple Hebrew greeting for reliability
...
<Response>
  ...
  <Record playBeep="true" maxLength="30" timeout="5" finishOnKey="*"/>
        ...
        log.info("Using Hebrew text greeting - should work reliably")
            
    except Exception as e:
        log.error("Call setup error: %s", e)
        # Fallback Hebrew greeting
        xml = """<?xml version="1.0" encoding="UTF-8"?>
<Response>
