...
interface AppointmentTypeManagementModalProps {
  isOpen: boolean;
  onClose: () => void;
  onTypeChange?: () => void;
}
...
const TEXTS = {
  ...
  subtitle: 'צור, ערוך ומחק את סוגי הפגישות של העסק',
  existing: 'סוגי פגישות קיימים',
  newType: 'סוג חדש',
  loading: 'טוען סוגי פגישות...',
  edit: 'עריכת סוג פגישה',
  create: 'יצירת סוג פגישה חדש',
  keyLabel: 'מפתח פנימי',
  keyPlaceholder: 'לדוגמה: viewing, meeting...',
  displayLabel: 'שם סוג הפגישה',
  displayPlaceholder: 'לדוגמה: צפייה, פגישה...',
  colorLabel: 'צבע',
  save: 'שמור סוג פגישה',
  cancel: 'ביטול',
  update: 'עדכן סוג פגישה',
  confirmDelete: 'מחיקת סוג פגישה',
  deleteConfirmText: 'האם אתה בטוח שברצונך למחוק את סוג הפגישה',
  deleteWarning: 'פעולה זו לא ניתנת לביטול',
  deleteError: 'שגיאה במחיקת סוג הפגישה',
  saveError: 'שגיאה בשמירת סוג הפגישה',
  emptyLabel: 'נא להזין שם לסוג הפגישה',
  emptyKey: 'נא להזין מפתח לסוג הפגישה',
  clickToEdit: 'לחץ לעריכה',
  deleteBtn: 'מחק',
  editBtn: 'ערוך',
  selectType: 'בחר סוג פגישה לעריכה',
  orCreateNew: 'או לחץ על "סוג חדש" ליצירה',
};
...
export default function AppointmentTypeManagementModal({ 
  isOpen, 
  onClose, 
  onTypeChange 
}: AppointmentTypeManagementModalProps) {
  const { types, loading, error, refreshTypes, updateTypes } = useAppointmentTypes();
  ...
  const handleEdit = (type: AppointmentTypeConfig, index: number) => {
    setFormData({
      key: type.key,
      label: type.label,
      color: type.color,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateTypes(updatedTypes);
      resetForm();
  ...
  const handleDeleteConfirm = async () => {
    ...
    setSaving(true);
    try {
      const updatedTypes = types.filter((_, idx) => idx !== deleteConfirmIndex);
      await updateTypes(updatedTypes);
      setDeleteConfirmIndex(null);
  ...
  const typeToDelete = deleteConfirmIndex !== null ? types[deleteConfirmIndex] : null;
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 overscroll-contain">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 lg:gap-6">
            {/* Type List - 3 columns on desktop */}
            <div className="lg:col-span-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                  {TEXTS.existing}
                  <span className="text-sm font-normal text-gray-500">({types.length})</span>
                </h3>
                <Button
              ...
              {loading ? (
                ...
                <div className="text-center py-12 bg-gray-50 dark:bg-gray-900/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                  ...
                  <p className="text-gray-500 mb-4 text-lg">אין סוגי פגישות עדיין</p>
                  <Button
                    onClick={handleStartCreate}
                              ...
                              <div className="flex items-center gap-3 flex-1">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600 dark:text-gray-400">
                                    {TEXTS.deleteConfirmText} "<strong>{typeToDelete.label}</strong>"?
                                  </p>
                            ...
                            <div className="p-3 sm:p-4">
                              <div className="flex items-center justify-between gap-3">
                                ...
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                  <Badge className={`${getColorClasses(type.color)} text-sm px-3 py-1`}>
                                    {type.label}
                                  </Badge>
            ...
            <div className="lg:col-span-2">
              {(isCreating || editingIndex !== null) ? (
                <div className="sticky top-0">
                  <Card className="p-4 sm:p-5 bg-white dark:bg-gray-800 shadow-lg border-2 border-purple-100 dark:border-purple-900/50">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="flex items-center gap-3 pt-2">
                        <Button
                          type="submit"
                          className="flex-1 bg-purple-600 hover:bg-purple-700 text-white min-h-[48px] text-base font-medium shadow-md hover:shadow-lg transition-all"
                          disabled={saving}
                        >
                          {saving ? (
