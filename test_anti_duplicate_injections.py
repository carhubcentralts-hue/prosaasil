...
4. Concurrent ensure calls don't create race condition duplicates
"""
import sys
import os
import threading
...
def test_concurrent_upgrade_no_duplicates():
    """
    Test 4: NEW - Verify concurrent ensure calls don't create race condition duplicates.
    
    Simulate 2 threads calling ensure_name_anchor simultaneously.
    Expected: Only 1 injection occurs (thread-safe).
    """
    print("\n" + "="*80)
    print("🔍 TEST 4: Concurrent Upgrade No Duplicates (Race Condition Test)")
    print("="*80)
    ...
    class MockSession:
        ...
        def ensure_name_anchor(self, name, policy, thread_id):
            """Thread-safe ensure with lock"""
            import hashlib
            new_hash_input = f"{name or 'None'}|{policy}"
            new_hash = hashlib.md5(new_hash_input.encode()).hexdigest()[:8]
            ...
            with self._lock:
                ...
                if existing_hash == new_hash:
                    print(f"   ℹ️ Thread {thread_id}: Skip (hash={existing_hash} already injected)")
                    return False
                ...
                self._name_anchor_hash = new_hash
                self._name_anchor_count += 1
                print(f"   ✅ Thread {thread_id}: Injected (count now={self._name_anchor_count}, hash={new_hash})")
                return True
    ...
    def thread_func(thread_id):
        result = session.ensure_name_anchor("דוד כהן", True, thread_id)
    ...
    # Verify only 1 injection occurred
    assert session._name_anchor_count == 1, f"Expected count=1 (race protected), got {session._name_anchor_count}"
    assert sum(results) == 1, f"Expected 1 successful injection, got {sum(results)}"
    
    print("\n✅ TEST 4 PASSED: No race condition duplicates!")
    return True
...
def main():
    ...
    try:
        ...
        test4_passed = test_concurrent_upgrade_no_duplicates()
        ...
        if test1_passed and test2_passed and test3_passed and test4_passed:
            ...
            print("   4. Concurrent ensure calls protected against race conditions")
            print("\n✅ Hash fingerprints prevent all duplicates:")
            print("   - system_hash: System prompt fingerprint (normalized)")
            print("   - business_hash: Business prompt fingerprint")
            print("   - name_hash: Name + policy fingerprint")
            print("   - Thread-safe: Lock prevents concurrent duplicates")
            print("="*80)
            return 0
        else:
            print("\n❌ SOME TESTS FAILED!")
