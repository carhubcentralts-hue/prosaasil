# 🔧 מדריך תיקון Google Cloud TTS

## הבעיה הנוכחית:
הsecret `GOOGLE_CLOUD_SERVICE_ACCOUNT_JSON` עדיין מכיל את שם הקובץ במקום את תוכן ה-JSON.

## פתרון צעד אחר צעד:

### 1. פתח את Replit Secrets
- בצד שמאל בReplit, לחץ על המנעול 🔒
- או לחץ על "Tools" ← "Secrets"

### 2. מצא את Secret הזה:
`GOOGLE_CLOUD_SERVICE_ACCOUNT_JSON`

### 3. מחק את התוכן הנוכחי
הוא אמור להיות: `manifest-alpha-465212-r7-bbfa7a03d904.json`

### 4. הדבק את התוכן הזה במדויק:
```json
{
  "type": "service_account",
  "project_id": "manifest-alpha-465212-r7",
  "private_key_id": "673b09e35c9b9d010cef4b1909ad96a42d167683",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n8xVMhUdvvHeInOq02O6G\n-----END PRIVATE KEY-----\n",
  "client_email": "tts-service@manifest-alpha-465212-r7.iam.gserviceaccount.com",
  "client_id": "116081385664114981196",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/tts-service%40manifest-alpha-465212-r7.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}
```

### 5. שמור
לחץ על "Save" או "Update"

### 6. אחרי השמירה המערכת תתאחל אוטומטית
ואז Google Cloud TTS יעבוד!

## זיהוי הצלחה:
כשזה יעבוד, תראה בלוגים:
- ✅ Google Cloud TTS client initialized successfully
- ✅ Found Hebrew voices available