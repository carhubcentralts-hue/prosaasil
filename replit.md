...
- **ALL ENGLISH PROMPTS (BUILD 324)**: MAJOR optimization - ALL system prompts, instructions, and rules now in ENGLISH for better OpenAI comprehension and lower token costs. AI still SPEAKS HEBREW to customers. Changes: (1) All greeting instructions in English, (2) All critical rules in English with English weekday names, (3) All fallback prompts in English, (4) Outbound prompts prepend "CRITICAL: SPEAK HEBREW to customer at all times", (5) Inbound critical rules include "SPEAK HEBREW naturally" as rule #1. DB content (greeting_message, ai_prompt) remains in Hebrew - only instructions are English.
