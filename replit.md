...
- **Hebrew STT Dictionary Expansion (BUILD 189)**: Comprehensive HEBREW_NORMALIZATION dictionary with 500+ corrections covering: numbers (כל הספרות בעברית), greetings (שלום, בוקר טוב), confirmations (כן, נכון, בסדר), negations (לא, אי אפשר), politeness (תודה, בבקשה), questions (למה, מתי, איפה), days/time (ימים, שעות), 70+ Israeli cities with all variations, 50+ professions/services (מנעולן, חשמלאי, אינסטלטור), appointments (פגישה, תור), 30+ common names, home/business/transport vocabulary. Dictionary-based approach fixes Whisper errors at transcription layer before AI processing.
