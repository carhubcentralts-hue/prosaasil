...
- **Hebrew STT Dictionary Expansion (BUILD 188)**: Expanded HEBREW_NORMALIZATION dictionary with 40+ city/service corrections. Fixes common Whisper errors at transcription layer: "כיפה/קיפה"→"חיפה", "תלביב/תלאביב"→"תל אביב", "זלטות/לתות/דלות"→"דלתות", "רמתגן"→"רמת גן", etc. Dictionary-based approach (not hardcoded prompt lists) enables real-time text normalization before AI processing. Prompt uses context-understanding instruction instead of static vocab lists.
