...
- **GREETING & PATIENCE FIX (BUILD 321)**: Critical fixes for natural conversation flow: (1) Strengthened greeting instruction with explicit "say EXACTLY this sentence word-for-word" and clear prohibition against reformulating, (2) Added "wait after greeting - let customer speak" instruction, (3) Added "critical patience" rules to full prompt: "don't ask 2 questions in a row, wait for complete answer before continuing", (4) Added patience rule to compact greeting prompt. Single source of truth: all greetings from `business.greeting_message` in DB.
- **PROMPT CONSOLIDATION (BUILD 322)**: Comprehensive audit and consolidation of all AI prompts: (1) Translated ALL English text to Hebrew in prompts (SCHEDULING → קביעת תורים, NO SCHEDULING → ללא קביעת תורים), (2) Unified ALL runtime fallback prompts to consistent Hebrew message: "אתה נציג שירות מקצועי. דבר בעברית, היה קצר ומועיל.", (3) Removed duplicate and conflicting prompt fragments, (4) Verified UI→DB→Runtime flow - all settings load from business configuration.
- **GREETING CONTEXT AWARENESS (BUILD 323)**: AI now explicitly knows the greeting IS the first question. When customer responds after greeting, AI understands the response is answering that greeting. Instruction added: "הברכה הזו היא גם השאלה הראשונה שלך! כשהלקוח עונה - התייחס לתשובתו כתגובה לברכה." Verified single source of truth: all settings (greeting_message, ai_prompt, required_lead_fields, enable_calendar_scheduling) flow from UI→DB→Runtime.
- **ALL ENGLISH PROMPTS (BUILD 324)**: MAJOR optimization - ALL system prompts, instructions, and rules now in ENGLISH for better OpenAI comprehension and lower token costs. AI still SPEAKS HEBREW to customers. Changes: (1) All greeting instructions in English, (2) All critical rules in English with English weekday names, (3) All fallback prompts in English, (4) Outbound prompts prepend "CRITICAL: SPEAK HEBREW to customer. If they don't understand Hebrew - switch to their language", (5) Inbound critical rules include "SPEAK HEBREW naturally. If customer speaks another language - switch to it" as rule #1. DB content (greeting_message, ai_prompt) remains in Hebrew - only instructions are English.
