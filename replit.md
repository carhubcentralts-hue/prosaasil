...
- **Cancelled Response Recovery (BUILD 187→192)**: When OpenAI cancels a response due to turn_detected BEFORE any audio is sent (output_count=0), the AI would go silent. BUILD 187 added recovery after speech_stopped. **BUILD 192 CRITICAL FIX**: When speech_started is blocked by grace period, speech_stopped never arrives → recovery never triggers! Fixed by triggering recovery IMMEDIATELY from response.done with cancelled (output_count=0), with 800ms delay and 3 guards (already speaking, response pending, user talking). Fixes the "AI goes completely silent" bug.
...
- **OpenAI VAD Tuning (BUILD 187→188)**: BUILD 187 was too aggressive (vad_threshold=0.9, silence=900ms) - users had to shout. BUILD 188 rebalanced: vad_threshold: 0.9→0.65 (normal conversation volume), silence_duration_ms: 900→600ms (faster AI response), prefix_padding_ms: 500ms (unchanged). Now detects normal speech and responds quickly.
