...
- **Production Fixes**: **BUILD 152**: Removed all hardcoded phone numbers - business identification is now fully dynamic via `Business.phone_e164` field. Added `VALIDATE_TWILIO_SIGNATURE` env var to control webhook signature validation (defaults to true). Added delete lead button in mobile CRM cards view. Added user deletion functionality with proper RBAC (system_admin and owner only).
- **Security Hardening**: **BUILD 153**: Fixed critical Twilio signature validation bypass - now only skippable in development mode (FLASK_ENV=development). Production mode rejects requests if TWILIO_AUTH_TOKEN is unset. Deprecated insecure /api/deals endpoints in api_payments.py (returns 410 with redirect to /api/crm/deals). Docker deployment uses Node 20+ for Baileys 7.x and Vite 7 compatibility.
- **Payments & Contracts Feature Freeze**: **BUILD 155**: Added `ENABLE_PAYMENTS` and `ENABLE_CONTRACTS` feature flags (default: false). All billing endpoints return 410 Gone when disabled. UI buttons/menu hidden. SearchModal mock invoice data commented out. Last hardcoded phone number in appointment handler replaced with dynamic business phone lookup. Production deployment ready with prosaas.pro domain configuration.
