...
def send_generic_webhook(
    ...
    🔥 Webhook Routing Logic:
        ...
        - Outbound calls: Use outbound_webhook_url, fallback to generic_webhook_url
        - Non-call events: Use generic_webhook_url
        
    This ensures all calls (inbound and outbound) can send webhooks even if specific
    URLs are not configured, falling back to the generic webhook URL.
    ...
    try:
        if not webhook_url:
            ...
            if direction == "outbound":
                # Outbound calls: Use outbound_webhook_url, fallback to generic
                    ...
                    while True:
                        ...
                        if response.status_code in (301, 302, 307, 308):
                            ...
                            if redirect_url:
                                ...
                                # Example with MAX_REDIRECTS=5:
                                # - Request 1: redirect_count becomes 1, check 1>5=no, follow
                                # - Request 2: redirect_count becomes 2, check 2>5=no, follow
                                # - Request 3: redirect_count becomes 3, check 3>5=no, follow
                                # - Request 4: redirect_count becomes 4, check 4>5=no, follow
                                # - Request 5: redirect_count becomes 5, check 5>5=no, follow
                                # - Request 6: redirect_count becomes 6, check 6>5=yes, BREAK
