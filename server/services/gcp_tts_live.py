...
class HebrewTTSLive:
    ...
    def synthesize_hebrew_pcm16_8k_streaming(self, text: str):
        """
        🚀 STREAMING TTS - yields audio chunks in real-time!
        First chunk arrives in ~200-300ms instead of 1.5s blocking delay
        
        Yields:
            bytes: PCM16 8kHz audio chunks (sent immediately to Twilio)
        """
        try:
            if not text.strip():
                return
            ...
            if not self._ensure_client():
                log.error("TTS client not available for streaming")
                return
            ...
            log.info(f"🎙️ STREAMING_TTS_START: {len(text)} chars")
            stream_start = time.time()
            
            # ✅ Split text into sentence chunks (~200 chars each for optimal streaming)
            chunks = self._split_into_chunks(text, max_chunk_size=200)
            log.debug(f"📦 Split into {len(chunks)} chunks: {chunks}")
            
            # ✅ Build streaming config (voice + audio settings)
            streaming_config = texttospeech.StreamingSynthesizeConfig(
                voice=self.voice,
                audio_config=self.audio_config_telephony
            )
            
            # ✅ Request generator: config first, then text chunks
            ...
            # ✅ Stream responses and yield audio chunks immediately!
            chunk_count = 0
            ...
            for response in streaming_responses:
                if response.audio_content:
                    ...
                    if chunk_count == 1:
                        ...
                        log.info(f"⚡ FIRST_CHUNK: {first_chunk_time:.3f}s ({len(response.audio_content)} bytes)")
                    
                    yield response.audio_content
            
            total_time = time.time() - stream_start
            if first_chunk_time is not None:
                log.info(f"✅ STREAMING_TTS_COMPLETE: {chunk_count} chunks, {total_bytes} bytes in {total_time:.3f}s (first: {first_chunk_time:.3f}s)")
            else:
                log.info(f"✅ STREAMING_TTS_COMPLETE: {chunk_count} chunks, {total_bytes} bytes in {total_time:.3f}s")
            
        except Exception as e:
            log.error(f"❌ STREAMING_TTS_ERROR: {e}")
            import traceback
            traceback.print_exc()
            return
