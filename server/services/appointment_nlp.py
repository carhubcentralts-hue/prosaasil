    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """אתה מנתח שיחות בעברית ומחלץ בקשות לקביעת פגישה.
החזר JSON בלבד עם השדות:
- action: "ask" (לקוח שואל על זמינות), "confirm" (לקוח מאשר שעה), או "none" (אין בקשה)
- date: תאריך בפורמט ISO (YYYY-MM-DD) או null. אם נאמר "מחר" - חשב לפי התאריך הנוכחי.
- time: שעה בפורמט HH:MM (24 שעות) או null. אם נאמר "בשש" זה 18:00, "בשבע וחצי" זה 19:30.
- name: שם הלקוח או null
