...
def _build_universal_system_prompt(call_direction: Optional[str] = None) -> str:
    ...
    base = (
        ...
        "  Hebrew: 'השתמש בשם הלקוח', 'השתמש בשם', 'פנה בשמו', 'קרא בשם', 'לפנות בשם', 'ליצור קרבה', 'באופן טבעי'\n"
        "  Conditional: 'אם קיים שם', 'במידה וקיים שם', 'אם יש שם'\n"
        "  English: 'use name', 'use their name', 'use customer name', 'address by name', 'call by name'\n"
        "- RULE: Any Business Prompt mentioning 'שם' or 'שם הלקוח' (customer name) with action verbs like השתמש/פנה/קרא means USE the CUSTOMER's NAME.\n"
