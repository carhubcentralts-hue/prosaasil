...
# 🔥 FIX: Add HTML document tags for full email documents from themes
ALLOWED_TAGS = [
    'a', 'b', 'blockquote', 'br', 'div', 'em', 'i', 'li', 'ol', 'p', 
    'strong', 'ul', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'table',
    'tbody', 'td', 'th', 'thead', 'tr',
    # HTML document structure tags (for theme-generated full documents)
...
def sanitize_html(html: str, is_full_document: bool = False) -> str:
    ...
    🔥 FIX: Skip sanitization for full HTML documents (from themes)
    Theme HTML is trusted (generated by our code in email_template_themes.py)
    Only sanitize HTML fragments (user input)
    ...
    Args:
        ...
        is_full_document: If True, skip sanitization (trusted theme HTML)
        
    Returns:
        Sanitized HTML or original if is_full_document=True
    """
    if is_full_document:
        # Full documents from themes are trusted - no sanitization needed
        return html
    
    # Sanitize HTML fragments (user input)
    if _HAS_CSS_SANITIZER:
        return bleach.clean(
            html,
            tags=ALLOWED_TAGS,
...
"""
    ...
    def send_crm_email(
        ...
        # 🔥 FIX: Check if HTML is already a full document BEFORE sanitization
        # Theme-generated emails are full HTML documents and should not be sanitized
        # (they're already safe - generated by our code, not user input)
        ...
        logger.info(f"[EMAIL_SEND] theme_id={settings.get('theme_id')} business_id={business_id} lead_id={lead_id} to={to_email} subject='{rendered_subject[:50]}'")
        logger.info(f"[EMAIL_SEND] is_full_document={is_full_document} html_length={len(rendered_body_html)}")
        
        if is_full_document:
            # HTML is already a complete document (from theme) - use as-is, no sanitization
            # Theme HTML is trusted (generated by our code in email_template_themes.py)
            logger.info(f"[EMAIL] HTML is full document from theme, skipping sanitization and base_layout wrapper")
            final_html_sanitized = rendered_body_html  # Use original, not sanitized
        else:
            # 4. Sanitize body content (user input only - fragments, not full documents!)
        ...
        # Plain text version
        final_text = rendered_body_text or strip_html(final_html_sanitized)
