...
2. **בדוק כל סטטוס** ברשימה וחפש התאמה:
   ...
   **עדיפות ראשונה - אין מענה / תקלות:**
   ...
   - אם בסיכום: "ניתק" / "התנתק" / "ניתוק מהיר" → חפש סטטוס עם "disconnect", "ניתק", "hung_up", "terminated"
   ...
   **עדיפות שנייה - עניין חיובי (רק אם יש ראיות ברורות!):**
   ...
   - אם בסיכום: "מעוניין" **ויש תוכן שיחה ממשי** → חפש סטטוס עם "interested", "מעוניין", "hot"
...
3. **היה גמיש וחכם:**
   - אותיות גדולות/קטנות לא משנות (case-insensitive)
   - קו תחתון (_) = רווח
   - חפש קשרים סמנטיים (למשל: "voicemail" ↔ "תא קולי")
   - אפילו אם הסטטוס נקרא "משהו_אחר_לגמרי", אם הוא מכיל מילה רלוונטית - זה התאמה!
...
5. **אם אין התאמה מושלמת:**
   - העדף סטטוסים שמכילים מילים רלוונטיות מהסיכום
   - העדף סטטוסים ניטרליים על פני "מעוניין" כשיש ספק
   - אם אין בכלל התאמה - החזר "none"

6. **חשוב מאוד:**
   - תמיד החזר את שם הסטטוס **בדיוק כמו שהוא ברשימה** (lowercase)
   - אל תשנה אותיות, אל תוסיף מילים
   - פשוט העתק את השם מהרשימה

🎯 **דוגמאות להתאמה חכמה:**
- סיכום: "הגיע לתא קולי" + יש סטטוס "voicemail_response" → החזר "voicemail_response"
- סיכום: "משיבון אוטומטי" + יש סטטוס "תא_קולי" → החזר "תא_קולי"
- סיכום: "לא נענה" + יש סטטוס "no_answer_2" והליד כבר ב-no_answer_1 → החזר "no_answer_2"
- סיכום: "ניתק באמצע" + יש סטטוס "call_dropped" → החזר "call_dropped"
- סיכום: "שיחה קצרה, 3 שניות" → **לא** "interested", אלא "no_answer"

**התשובה שלך (רק שם הסטטוס מהרשימה):**"""
...
🎯 **המשימה שלך: למצוא את ההתאמה הכי טובה!**

⚠️ **חשוב מאוד - היה שמרני ומדויק:**
- אל תבחר "interested" או "מעוניין" אלא אם יש ראיות ברורות לעניין אמיתי!
- רוב השיחות הן "no answer", "disconnected", או "contacted" - לא "interested"
...
אתה צריך למצוא איזה סטטוס **הכי מתאים** לסיכום.
...
2. אם זה שיחה קצרה (< 15 שניות) או אין מענה:
   ...
   - "ניתק" / "התנתק" בסיכום ↔ "disconnect", "ניתק", "hung_up" בסטטוס

**עדיפות שנייה - עניין חיובי (רק עם ראיות!):**
3. רק אם יש שיחה ממשית ועניין ברור:
   - "מעוניין" **+ תוכן שיחה** בסיכום ↔ "interested", "מעוניין", "hot" בסטטוס
   - "נקבעה פגישה" בסיכום ↔ "appointment", "meeting", "פגישה" בסטטוס
   - "לא מעוניין" בסיכום ↔ "not_interested", "לא_מעוניין" בסטטוס

**עדיפות שלישית - מצבים אחרים:**
4. **חכם סמנטי:** גם אם המילים לא זהות - חפש קשרים:
...
5. **היה גמיש:**
   ...
   - קו תחתון = רווח
   - חפש חלקים של מילים גם (למשל: "disconnect" מכיל את "connect")

6. **תעדיף סטטוסים מדויקים:**
   - אם יש "voicemail_1" ו-"no_answer", ובסיכום "תא קולי" → בחר "voicemail_1"
   - אם יש "disconnected_mid_call" ו-"contacted", ובסיכום "ניתק באמצע" → בחר "disconnected_mid_call"
   - אם יש ספק בין "interested" ו-"contacted" → בחר "contacted" (השמרני יותר!)
