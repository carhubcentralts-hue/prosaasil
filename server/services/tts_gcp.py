...
def create_tts_client() -> Optional[texttospeech.TextToSpeechClient]:
    ...
    try:
        ...
        if not sa_json:
            logger.error("❌ GOOGLE_CLOUD_SERVICE_ACCOUNT_JSON not found")
            return None
        ...
        logger.info("✅ Google Cloud TTS client created")
        return client
...
def synthesize_hebrew_to_mp3(text: str, output_path: str, voice: str = "he-IL-Wavenet-A") -> bool:
    ...
    try:
        ...
        logger.info(f"✅ Hebrew TTS saved to: {output_path}")
        return True
    ...
    try:
        ...
        for file_path, text in greetings.items():
            if not os.path.exists(file_path):
                logger.info(f"🎯 Creating Hebrew greeting: {file_path}")
        ...
        logger.error(f"❌ Error ensuring greeting files: {e}")
