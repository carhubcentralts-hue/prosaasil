...
הנחיות לסיכום (חובה לכלול):
1. **נושא השיחה** - מה הלקוח רצה/ביקש
2. **מה נדון** - הנקודות העיקריות שעלו בשיחה
3. **תוצאה** - מה סוכם או מה קרה בסוף (כולל אם הלקוח ניתק/סיים)
4. **המשך** - האם יש פעולה נדרשת או המשך מתוכנן
5. **המלצת סטטוס** - המלצה לסטטוס חדש של הליד על סמך השיחה

כללים:
- כתוב רק מה שבאמת נאמר - אל תמציא מידע
...
- המלצת סטטוס צריכה להיות מילה אחת בעברית (למשל: מעוניין, לא_רלוונטי, נקבע_פגישה, חזרה_נדרשת, לא_ענה)

דוגמאות לסיכומים טובים (עם המלצת סטטוס):
- "לקוח פנה בנושא ניקוי מזגן. ביקש לדעת מחירים. סוכם לחזור אליו עם הצעת מחיר. [המלצה: מעוניין]"
- "לקוח התקשר לברר על שירות. אחרי שאלה אחת ביקש לסיים את השיחה - לא התעניין בשלב זה. [המלצה: לא_מעוניין]"
- "תזכורת לפגישה מחר בשעה 10. הלקוח אישר הגעה. [המלצה: פגישה_מאושרת]"
- "לקוח שאל על זמינות. לא מצאנו תאריך מתאים - יחזור אלינו בשבוע הבא. [המלצה: חזרה_נדרשת]"
- "שיחה לא נענתה. הגיע לתא קולי. [המלצה: לא_ענה]"

חשוב: הוסף את המלצת הסטטוס בסוף הסיכום בפורמט [המלצה: <סטטוס>]
...
2. שימושי - מידע שעוזר להבין מה קרה ומה הצעד הבא
3. כן - אם הלקוח לא התעניין או ניתק, תכתוב את זה במפורש
4. מלא - כולל המלצה לסטטוס חדש בהתאם לתוכן השיחה

מבנה הסיכום:
- נושא הפנייה
- מה נדון
- התוצאה (כולל אם לא הגיעו לסיכום)
- המשך מתוכנן (אם יש)
- [המלצה: <סטטוס>] - בסוף הסיכום

סטטוסים אפשריים להמלצה:
- מעוניין (לקוח הביע עניין)
- לא_מעוניין / לא_רלוונטי (לקוח לא מעוניין או מספר לא נכון)
- נקבע_פגישה / פגישה_מאושרת (נקבעה פגישה)
- חזרה_נדרשת (צריך לחזור ללקוח)
- לא_ענה / אין_מענה (לא ענה או תא קולי)
- נוצר_קשר (השיחה התבצעה אך ללא תוצאה ברורה)

דוגמאות:
✓ "לקוח פנה לגבי ניקוי מזגן. שאל על מחירים. ניתק לפני שקיבל מידע - לא התקדם. [המלצה: לא_מעוניין]"
✓ "בירור על שירות הובלה. הלקוח ביקש הצעת מחיר. סוכם לשלוח הצעה במייל. [המלצה: מעוניין]"
✓ "תזכורת לפגישה מחר. הלקוח אישר הגעה בשעה 10. [המלצה: פגישה_מאושרת]"
✓ "שיחה לא נענתה - הגיע לתא קולי אוטומטי. [המלצה: לא_ענה]"
✗ "שיחה טובה עם לקוח מעוניין" (חסר פרטים ואין המלצת סטטוס)

אורך: 30-80 מילים לרוב השיחות + שורת המלצה."""
                },
