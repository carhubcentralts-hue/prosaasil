...
def live_call_chat():
    """
    Chat processing for live call (OpenAI brain - always)
    ...
    try:
        ...
        # Call OpenAI Chat (brain is always OpenAI)
        from openai import OpenAI
        ...
        logger.info(f"[LIVE_CALL][CHAT] User: {text[:50]}... → AI: {ai_response[:50]}...")
...
def live_call_tts():
    ...
    try:
        ...
        logger.info(f"[LIVE_CALL][TTS] Synthesized {len(audio_bytes)} bytes with {provider}/{voice_id}")
