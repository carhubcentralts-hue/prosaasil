...
class MediaStreamHandler:
    ...
    async def _run_realtime_mode_async(self):
        ...
        try:
            ...
            # 🔥 BUILD 315: Build FULL business prompt FIRST (not in background!)
            # This is the key fix - AI has full context from the very beginning
            full_prompt = None
            try:
                from server.services.realtime_prompt_builder import build_realtime_system_prompt as build_prompt
                app = _get_flask_app()
                with app.app_context():
                    full_prompt = build_prompt(business_id_safe, call_direction=call_direction)
                    print(f"✅ [BUILD 315] Full prompt built SYNCHRONOUSLY: {len(full_prompt)} chars")
            except Exception as prompt_err:
                print(f"⚠️ [BUILD 315] Failed to build full prompt: {prompt_err}")
                full_prompt = None
            
            # 🔥 BUILD 315: Construct greeting instruction based on business AI prompt
            # The greeting should be GENERATED by the AI based on the full context!
            if call_direction == 'outbound' and outbound_lead_name:
                # OUTBOUND: Use template greeting
                ...
                if not outbound_greeting:
                    outbound_greeting = f"{outbound_lead_name}, שלום"
                
                greeting_instruction = f"""
🎯 משימה ראשונה: פתח בברכה יוצאת ללקוח
ברכה: "{outbound_greeting}"
"""
                print(f"📤 [OUTBOUND] Using greeting: {outbound_greeting[:50]}...")
            else:
                # 🔥 BUILD 315: INBOUND - AI generates greeting from prompt context!
                # No static greeting text - AI greets based on its full understanding
                if greeting_text and greeting_text.strip():
                    greeting_instruction = f"""
🎯 משימה ראשונה: פתח בברכה ללקוח
ברכה מוגדרת: "{greeting_text}"
"""
                    print(f"📞 [INBOUND] Using configured greeting: {greeting_text[:50]}...")
                else:
                    # No greeting configured - AI will greet naturally based on prompt
                    greeting_instruction = f"""
🎯 משימה ראשונה: הזדהה כנציג של {biz_name} ושאל במה תוכל לעזור
(פתח בקצרה, אל תקרא את כל הפרומפט)
"""
                    print(f"📞 [INBOUND] AI will generate natural greeting for: {biz_name}")
            
            # 🔥 BUILD 315: Combine full prompt + greeting instruction
            # This gives AI complete context from the first moment!
            if full_prompt:
...
{full_prompt}"""
                ...
                # Fallback: No full prompt - use minimal greeting
...
חוקים: קצר מאוד, המתן לתשובת הלקוח."""
            ...
            # 🔥 BUILD 315: PHASE 2 SIMPLIFIED - Only add lead capture tool
            # Full prompt already sent in Phase 1 for complete context from start!
            async def _add_lead_capture_tool():
                try:
                    # Wait for greeting to finish before adding tool
                    wait_start = time.time()
                    max_wait_seconds = 15
                    ...
                    # 🔥 BUILD 315: Only add the lead capture tool - prompt already sent!
                    ...
                    if lead_tool:
                        ...
                        session_config = {
                            "tools": [lead_tool],
                            "tool_choice": "auto",
                            "voice": voice_to_use  # Re-send voice to keep it locked
                        }
                        ...
                        print(f"✅ [BUILD 315] Added lead capture tool for fields: {getattr(self, 'required_lead_fields', [])}")
                    else:
                        print(f"ℹ️ [BUILD 315] No lead fields required - skipping tool add")
                        
                except Exception as e:
                    print(f"⚠️ [BUILD 315] Tool add error: {e}")
            
            # Start tool addition in background (non-blocking)
            asyncio.create_task(_add_lead_capture_tool())
