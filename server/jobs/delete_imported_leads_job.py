"""
Delete Imported Leads Job
Background job for stable, batched deletion of imported leads
...
def _release_bulk_gate_lock(business_id: int):
    """
    Helper function to release BulkGate lock for delete_imported_leads operation.
...
def delete_imported_leads_batch_job(job_id: int, business_id: int = None, **kwargs):
    """
    Background job for deleting imported leads in batches
    
    This runs in a separate worker process and:
    1. Loads job state from database
    2. Processes leads in batches of BATCH_SIZE
    3. Deletes only leads with source="imported_outbound"
    ...
    print(f"=" * 70)
    print(f"🔨 JOB PICKED: function=delete_imported_leads_batch_job job_id={job_id}")
    print(f"=" * 70)
    logger.info(f"=" * 70)
    logger.info(f"🔨 JOB PICKED: queue=maintenance function=delete_imported_leads_batch_job job_id={job_id}")
    logger.info(f"=" * 70)
